---
layout: portfolio
title: Contact
permalink: /portfolio/contact/
description: Get in touch with Satyajit Roy Choudhury regarding engineering leadership, speaking, or advisory roles.
---

<div class="p-container">
  <div class="p-section" style="border-bottom: none; padding-bottom: 2rem;">
    <h1>Let's Connect</h1>
    <p class="p-subhead">Open to discussing strategic engineering leadership, architectural advisory, and speaking opportunities.</p>
  </div>

  <div class="p-grid-2" style="gap: 2rem; margin-bottom: 4rem;">
    <div class="p-card" style="border-left: 4px solid #10b981;">
      <h3 style="font-size: 1.1rem; margin-bottom: 1rem;"><i class="fas fa-briefcase" style="margin-right: 0.5rem; color: #10b981;"></i> Executive Leadership</h3>
      <p style="font-size: 0.95rem; color: var(--color-text);">Interested in Director/VP roles focusing on Engineering, Platform Engineering, and Site Reliability Engineering at high-growth organizations.</p>
    </div>
    <div class="p-card" style="border-left: 4px solid #3b82f6;">
      <h3 style="font-size: 1.1rem; margin-bottom: 1rem;"><i class="fas fa-microphone" style="margin-right: 0.5rem; color: #3b82f6;"></i> Speaking & Workshops</h3>
      <p style="font-size: 0.95rem; color: var(--color-text);">Available for keynote sessions or technical workshops on SRE culture, cloud cost optimization, and AI/ML infrastructure.</p>
    </div>
    <div class="p-card" style="border-left: 4px solid #f59e0b;">
      <h3 style="font-size: 1.1rem; margin-bottom: 1rem;"><i class="fas fa-chart-line" style="margin-right: 0.5rem; color: #f59e0b;"></i> Strategic Advisory</h3>
      <p style="font-size: 0.95rem; color: var(--color-text);">Consulting for startups on building resilient technical foundations, FinOps governance, and scaling high-performance teams.</p>
    </div>
    <div class="p-card" style="border-left: 4px solid #8b5cf6;">
      <h3 style="font-size: 1.1rem; margin-bottom: 1rem;"><i class="fas fa-comments" style="margin-right: 0.5rem; color: #8b5cf6;"></i> Mentorship</h3>
      <p style="font-size: 0.95rem; color: var(--color-text);">Passionate about developing the next generation of engineering leaders and architects in the distributed systems space.</p>
    </div>
  </div>

  <div class="p-section" style="background: var(--color-bg-alt); padding: 4rem 2rem; border-radius: 12px; text-align: center;">
    <h2 style="margin-bottom: 1rem;">Uplink Channels</h2>
    <p style="margin-bottom: 3rem; color: var(--color-text);">Preferred method of contact is via LinkedIn or Email.</p>
    
    <div class="c-socials" style="margin-top: 0; gap: 3rem;">
      <a href="mailto:talk2sroy.ch@gmail.com" title="Email" style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem; border: none;">
        <div style="width: 60px; height: 60px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); color: #ea4335;">
          <i class="fas fa-envelope"></i>
        </div>
        <span style="font-size: 0.8rem; font-weight: 700; color: var(--color-heading);">EMAIL</span>
      </a>
      <a href="https://www.linkedin.com/in/satyajitroychoudhury/" target="_blank" title="LinkedIn" style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem; border: none;">
        <div style="width: 60px; height: 60px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); color: #0077b5;">
          <i class="fab fa-linkedin"></i>
        </div>
        <span style="font-size: 0.8rem; font-weight: 700; color: var(--color-heading);">LINKEDIN</span>
      </a>
      <a href="https://github.com/kodelint" target="_blank" title="GitHub" style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem; border: none;">
        <div style="width: 60px; height: 60px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); color: #333;">
          <i class="fab fa-github"></i>
        </div>
        <span style="font-size: 0.8rem; font-weight: 700; color: var(--color-heading);">GITHUB</span>
      </a>
      <a href="https://twitter.com/__initialized__" target="_blank" title="Twitter/X" style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem; border: none;">
        <div style="width: 60px; height: 60px; background: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.05); color: #000;">
          <i class="fab fa-x-twitter"></i>
        </div>
        <span style="font-size: 0.8rem; font-weight: 700; color: var(--color-heading);">TWITTER</span>
      </a>
    </div>
  </div>
</div>